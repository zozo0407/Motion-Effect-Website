...
function createShader() {
    ...
    return {
        name: 'ExpandTileShaderPass',
        uniforms: {
            'tDiffuse': { value: null },
            'progress': { value: 0.1 },
            'ratio': { value: 1.0 },
            'tile_count': { value: 4.0 },
        },
...
/**
 * ExpandTileShaderPass类
 * 继承自Three.js的ShaderPass，提供expandTile效果
 */
class ExpandTileShaderPass extends ShaderPass {
  constructor() {
    super(createShader());
  ...
  /**
   * 设置瓦片数量
   * @param {number} tile_count - 瓦片数量
   */
  setTileCount(tile_count) {
    this.tile_count = this._clampValue(tile_count, 0, 4);
  }
  ...
  render(renderer, writeBuffer, readBuffer, delta, maskActive) {
    this.material.uniforms.tile_count.value = Math.max(1, Math.round(this.intensity * (this.tile_count - 1) + 1));
...
/*threeJS*/ module.exports = { ExpandTileShaderPass };
// /*Effect*/ exports.ExpandTileShaderPass = ExpandTileShaderPass;
