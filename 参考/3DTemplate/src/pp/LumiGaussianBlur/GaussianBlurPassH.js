...
// 导入Shader源码
const vertexShaderX = require('./shaders/gaussianBlurXVert.js').source;
const fragmentShaderX = require('./shaders/gaussianBlurXFrag.js').source;

// 创建X方向Shader
function createShaderX() {
    const vertexShader = vertexShaderX
    ...
    const fragmentShader = fragmentShaderX
        .replace(/uv0/g, 'v_uv')
        .replace(/u_inputTexture/g, 'tDiffuse')
        .replace(/gl_FragData\[0\]/g, 'gl_FragColor');
    
    return {
        name: 'GaussianBlurPassH',
        uniforms: {
            'tDiffuse': { value: null },
            'u_sampleX': { value: 10.0 },
            'u_sigmaX': { value: 1.0 },
            'u_stepX': { value: 0.001 },
            'u_stepY': { value: 0.001 },
            'u_gamma': { value: 2.2 },
 ...
 * X方向高斯模糊ShaderPass类
 */
class GaussianBlurPassH extends ShaderPass {
    /**
     * 构造函数
     */
    constructor() {
...
/*threeJS*/ module.exports = { GaussianBlurPassH };
// /*Effect*/ exports.GaussianBlurPassH = GaussianBlurPassH;
