...
var now = function () {
    ...
    if (typeof process !== 'undefined' && process.hrtime) {
        if (!now._nodeStartTime) {
            now._nodeStartTime = process.hrtime();
        }
        var hrTime = process.hrtime(now._nodeStartTime);
        return hrTime[0] * 1000 + hrTime[1] / 1000000;
    }
    
    // 浏览器兜底：模拟performance.now()行为
    if (typeof now._startTime === 'undefined') {
        now._startTime = Date.now();
        now._lastReturned = 0;
    }
    
    var elapsed = Date.now() - now._startTime;
    
    // 确保时间单调递增，同时添加微小递增来模拟高精度
    if (elapsed <= now._lastReturned) {
        now._lastReturned += 0.001;
    } else {
        now._lastReturned = elapsed;
    }
    
    return now._lastReturned;
};
