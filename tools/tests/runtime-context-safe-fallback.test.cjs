...
const autofix = fs.readFileSync(path.join(root, 'server', 'services', 'code-autofix.js'), 'utf8');
...
assert(!autofix.includes('this.scene = ctx.scene;'), 'server auto-fix must not inject ctx.scene usage');
assert(autofix.includes('new THREE.Scene()'), 'server auto-fix should create its own THREE.Scene');
